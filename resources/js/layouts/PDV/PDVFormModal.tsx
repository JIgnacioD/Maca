...
export default function PDVFormModal({ isOpen, isEdit, data = {}, onClose, onSave }: Props) {
    ...
    // Solo actualiza el formulario si `data` cambia y es diferente al estado actual
    React.useEffect(() => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form.pdv_name || !form.address || form.lat === undefined || form.lng === undefined) {
        ...
        try {
            ...
            if (!csrfToken) {
                console.error('El token CSRF no está disponible.');
                alert('No se puede completar la solicitud porque falta el token CSRF.');
                return;
            }

            if (isEdit && !form.id) {
                console.error('El ID del PDV no está definido.');
                alert('No se puede editar el PDV porque falta el ID.');
                return;
            }

            const url = isEdit ? `/pdvs/${form.id}` : '/pdvs';
            const method = isEdit ? 'PUT' : 'POST';

            console.log('Enviando solicitud:', { url, method, body: form });
            ...
            console.error('Error al guardar el PDV:', error);
            alert('Hubo un error al guardar el PDV.');
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-background rounded-2xl shadow-lg w-full max-w-lg p-6">
